...
int main(int argc, char** argv) {
    ...
    */
	//al_play_sample(themeSound, 0.5f, 0.0f, 1.0f, ALLEGRO_PLAYMODE_LOOP, NULL);
        ...
        for (int i = 0; i < 2 && al_get_next_event(eventQueue, &event); ++i) {
            if (event.type == ALLEGRO_EVENT_KEY_UP) {
                ...
                switch (event.keyboard.keycode) {
                    case ALLEGRO_KEY_UP:
                    state |= UP;
                    gPlayer->jumpHeight = 0;
                    al_play_sample(jumpSound, 1.0f, 0.0f, 1.0f, ALLEGRO_PLAYMODE_ONCE, &jumpSoundId);
                    break;
                    case ALLEGRO_KEY_SPACE:
                    state |= UP;
                    gPlayer->jumpHeight = 0;
